...
class BadgeOS {
	...
	function __construct() {
        ...
        $this->start_time	  = current_time( 'timestamp' ) - 5 ;//  strtotime(" -1 second " );

        $this->award_ids  = array();

		add_filter( 'plugin_action_links_' . $this->basename, [ $this, 'settings_link' ], 10, 1 );
	...
	function activation_redirect( $plugin, $network_wide ) {
		if( trim( $this->basename ) == trim( $plugin ) ) {
			exit( wp_redirect( 'admin.php?page=badgeos-templates'  ) );
		}
	}
	...
	function settings_link( $links ) {
		//print_r($links);
		$settings_link = '<a href="' . admin_url('admin.php?page=badgeos_settings') . '">'. __( 'Settings', 'badgeos' ). '</a>';
		//$links[] = $settings_link;
	...
	function includes() {
		...
		//Template redirect hook
		//require_once( $this->directory_path . 'includes/plugin-templates.php' );
	...
	function register_scripts_and_styles() {
        ...
        $admin_js_translation_array = array(
			...
			'badgeos_templates_log'     => __( 'Installation Log', 'badgeos' ),
		...
		// Template js files
		// wp_register_script('badgeos-carouselTicker-js', $this->directory_url . 'js/jquery.carouselTicker.js',"jquery", self::$version, true);
		// wp_register_script('badgeos-templates-js', $this->directory_url . 'js/templates.js',"jquery", self::$version, true);
...
//			update_option( 'credly_settings', $credly_settings );
	...
	function plugin_menu() {
		...
		//if( !isset( $settings['wizard_already_run'] ) && $settings['wizard_already_run'] != 'yes' ) {
		//add_submenu_page( 'badgeos_badgeos', __( 'Templates', 'badgeos' ), __( 'Templates', 'badgeos' ), $main_item_role, 'badgeos-templates', 'badgeos_templates' ); 
		//}
