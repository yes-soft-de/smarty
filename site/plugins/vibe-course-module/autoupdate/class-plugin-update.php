...
class Vibe_Course_Module_Auto_Update{
	...
	public function plugins_api_result( $res, $action, $args ) {
		...
		if ( $plugin_slug == $updater_data['slug'] && $action == 'plugin_information' ){
			...
			if ( is_wp_error( $request ) ) {
				$res = new WP_Error( 'plugins_api_failed', '<p>' . __( 'An Unexpected HTTP Error occurred during the API request.', 'vibe' ) . '</p><p><a href="?" onclick="document.location.reload(); return false;">' . __( 'Try again', 'vibe' ) . '</a></p>', $request->get_error_message() );
			}
	...
	public function dashboard_widget_callback() {
		...
		/* Get needed data */
		$updater_data = $this->updater_data();
		//$token = get_option('envato_token'); access_token expires refresh_token
		/* Widget ID, prefix with "ahp_" to make sure it's unique */
		$widget_id = 'ahp_' . $updater_data['slug'] . '_activation_key';
