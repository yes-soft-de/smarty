...
var LS_SliderLibrary = {
	...
	init: function( userSettings ) {
		...
		if( ! LS_SliderLibrary.initialized ) {
			...
			// Open group
			$( document ).on('click', '.slider-item.group-item', function( e ) {
				e.preventDefault();
				LS_SliderLibrary.openGroup( $( this ) );
	...
	parseData: function( userSettings ) {
		...
		LS_SliderLibrary.settings = $.extend( true, {}, LS_SliderLibrary.defaults, userSettings );
	},
	...
	open: function( userSettings ) {
		...
		$.get( ajaxurl, { action : 'ls_slider_library_contents' }, function( html ) {
			$('#ls-slider-library-modal .kmw-modal-content').html( html );
		});
	},
	...
	selectSlider : function( event, $item ) {
		...
		var sliderData = $.extend( true, {}, $item.data() );
