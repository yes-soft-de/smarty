...
class SimpleJWTLoginSettings {
	...
	private function validateGeneralSettingsFromPost() {
		...
		if(!empty($this->post['jwt_algorithm'])){
			if(strpos($this->post['jwt_algorithm'], 'RS') !== false){
				if (
					...
					throw  new \Exception( __( 'JWT Decryption public and private key are required.', 'simple-jwt-login' ) );
				}
			} else {
		...
		if (
			...
			throw  new \Exception( __( 'JWT Parameter key from Login Config is missing.', 'simple-jwt-login' ) );
		}
		if (
