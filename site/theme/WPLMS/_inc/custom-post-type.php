<?php
	...
	function smarty_consultation_column_value( $column, $post_id ) {
		switch ( $column ) {
			...
			case 'price' :
				...
				echo '<span class="_price">' . $price . '</span>';
				break;
			case 'email':
				$email = get_post_meta( $post_id, '_consultation_email_value_key', true );
	...
	function smarty_consultation_email_callback( $post ) {
		...
		$value = get_post_meta( $post->ID, '_consultation_email_value_key', true );
	...
	function smarty_consultation_price_callback( $post ) {

		wp_nonce_field( 'smarty_save_consultation_price_data', 'smarty_consultation_price_meta_box_nonce' );

		$value = get_post_meta( $post->ID, '_price', true );
		// Print The input as wordpress theme build-in
		echo '<label for="_price">' . __('Consultation Price') . ': </label>';
		echo '<input type="number" id="_price" name="_price" value="' . esc_attr( $value ) . '" size="25" />';
	}
	...
	function smarty_save_consultation_email_data( $post_id ) {
		...
		update_post_meta( $post_id, '_consultation_email_value_key', $my_data );
	}

	// function to print the email inside our meta box and save the post
	function smarty_save_consultation_price_data( $post_id ) {
		// check if the nonce is not set in the $_POST Method
		if ( ! isset( $_POST['smarty_consultation_price_meta_box_nonce'] ) ) {
			return;
		}
		// check if the nonce is valid and nonce is generated by wordpress and not generated by and hacker or another user
		// sunset_save_contact_email_data: is the actual function that saving my meta box
		if ( ! wp_verify_nonce( $_POST['smarty_consultation_price_meta_box_nonce'], 'smarty_save_consultation_price_data' ) ) {
			return;
		}
		// check is a manual save or automatically save | prevent the wordpress from saving by him self the meta box
		if ( defined('DOING_AUTOSAVE') && DOING_AUTOSAVE ) {
		...
		// Retrieve the data
		$my_data = sanitize_text_field( $_POST['_price'] );
