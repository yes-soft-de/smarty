...
function smart_way_load_more() {
	...
	$course = bp_course_get_full_course_curriculum( $courseId ); 
		...
		foreach ( $course as $lesson ):
			...
			if ( $lesson['type'] == 'unit' ):
				if ( wp_get_attachment_url( get_post_thumbnail_id( $lesson['id'] ) ) ) {
					...
					$image = get_template_directory_uri() . '/assets/img/inner-peace-meditation.jpg';
				}
				$output .= '
				<li class="llms-loop-item mx-auto meditations ' . ($lesson['labels'] != null ? 'freeLesson' : '') . '" style="padding: 1.7rem; margin-top: 1rem; margin-bottom: 1rem;">
				<div class="llms-loop-item-content" style="margin: 0;">
					...
					<div class="col-xs-7">
						<a class="llms-loop-link ' . ($lesson['labels'] == null ? 'paidCourse' : '') . '" href="' . get_the_permalink( $lesson['id'] ) . '">
							...
							<span class="meditation-shadow-title" style="display: block">' . $lesson['title'] . '</span>
							</div>
						</a><!-- .llms-loop-link -->
					</div>
